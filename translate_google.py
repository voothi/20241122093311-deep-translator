...
def main():
    ...
    try:
        try:
            translator = GoogleTranslator(source=args.source, target=args.target, **translator_kwargs)
        except TypeError:
            try:
                kwargs_no_session = {k: v for k, v in translator_kwargs.items() if k != "session"}
                translator = GoogleTranslator(source=args.source, target=args.target, **kwargs_no_session)
            except TypeError:
